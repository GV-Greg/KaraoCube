<div class="w-full mt-5 lg:w-3/6 md:w-4/6 sm:w-5/6 relative" x-data="{ open: true, openForm: false }">
    ...
    <div class="absolute md:px-10 w-full flex justify-center">
        @if(strlen($query) > 2)
                    ...
                    <ul>
                        @foreach($songs as $index => $song)
                            <li class="p-1 hover:text-green-500">
                                <button @click="openForm = true; @this.showForm({{$index}}); open = false;" @click.away="openForm = false" class="cursor-pointer text-left">{{ $song->interpreter }} - {{ $song->title }}</button>
                            </li>
                        @endforeach
                    </ul>
                @else
                    <div class="w-full p-3 flex flex-col justify-center">
                        <span class="md:mt-1.5 text-red-500 text-center italic">O r&eacute;sultat pour {{ $query }}</span>
